...
| **Reactivity correctness (data/computed/watch)** | **vue-reviewer** |
| **`v-html` audit, template injection, unsafe URL binding** | **vue-reviewer** |
| **Mixin merge collisions, side effects, cleanup** | **vue-reviewer** |
...
| **Render performance, v-memo, markRaw, v-once** | **vue-reviewer** |
...
- **`data` as an object literal instead of a function**: `data: { count: 0 }` shares one state object across every instance of the component. Must be `data() { return { count: 0 } }`. Vue 3 throws for components, but a mixin or `extends` base can still smuggle it in.
- **State assigned to `this` without being declared in `data()`**: `this.results = []` set for the first time inside a method or lifecycle hook is a plain property — not reactive, never re-renders. Declare it in `data()` (as `null` if needed).
- **Arrow functions for `data` / `computed` / `methods` / lifecycle hooks**: `this` is not the component instance — silently `undefined`. Only genuine standalone helpers may be arrows.
- **Mutating a prop**: `this.someProp = x` breaks one-way data flow and warns in development. Copy into `data()` for a local draft or emit up.
- **`v-model` bound to a read-only computed**: A `computed` with only a getter drops writes silently. Use the `{ get, set }` object form.
- **Non-reactive external objects placed in `data()`**: Chart instances, WebSocket clients, and observers get deep-proxied — expensive and often breaks the library's identity checks. Assign them to `this` in `created`/`mounted`, or wrap in `markRaw()`.
- **Vue 2 reactivity workarounds left in place**: `this.$set`, `this.$delete`, and `Vue.set` do not exist in Vue 3. Direct assignment is reactive via Proxy.

### HIGH — Mixins and Shared Logic

- **Mixin state or method colliding with the host component**: On a key clash the component silently wins for `methods`/`computed`, and `data` merges shallowly. Neither warns. Require namespaced mixin keys (`timer_remaining`, not `remaining`).
- **Mixin without cleanup**: Timers, listeners, subscriptions, and in-flight requests started in a mixin's `created`/`mounted` must be torn down in the mixin's own `beforeUnmount` — not left to the host component.
- **Module-scope side effects in a shared module**: Initializing state, starting timers, or subscribing at import time means the side effect is shared by every component that imports it.
- **Grab-bag mixin (`commonMixin`, `utilsMixin`)**: Multiple unrelated concerns in one mixin makes merge order and ownership unknowable. Split by concern.
- **Mixin depending on undeclared host properties**: A mixin reading `this.itemId` without documenting that requirement fails silently on the next component that adopts it.
- **Stateless logic implemented as a mixin**: Pure functions with no `data`/`computed`/lifecycle needs belong in a plain module imported into `methods` — a mixin adds merge risk for nothing.
...
- **Large Single-File Component (>300 lines template + script)**: Extract subcomponents or shared modules. Long SFCs hurt readability, testability, and tree-shaking.
- **Props mutation**: Modifying props directly (even reactive objects) is forbidden — Vue warns in development. Emit a declared event to communicate up, or use `v-model`.
- **Missing prop validation**: Every prop should have at minimum `type`, plus `required`/`default` where appropriate. Object and Array defaults must be factory functions (`default: () => []`) — a shared literal leaks state across instances.
- **Missing `emits` option**: Undeclared events fall through to the root element as native listeners and skip validation. Every `this.$emit` needs a matching `emits` entry.
...
- **Direct DOM manipulation via `document.querySelector`**: Use `this.$refs.name` with `ref="name"` in the template. Raw DOM selectors break component encapsulation. `$refs` is only populated from `mounted` onward.
- **Component missing a `name` option**: Breaks devtools display, recursive self-reference, and `<KeepAlive :include>` matching.
...
- **Route params copied into `data()` in `created`**: Params change on navigation within the same mounted component — the copy goes stale. Read `this.$route.params` in a `computed`, and refetch from `watch: { '$route.params.id': ... }`.
- **Missing `beforeRouteUpdate`**: Reusing a component across param changes without `beforeRouteUpdate` (or a `$route` watcher) leaves the previous record's data on screen.
- **`this` used in `beforeRouteEnter`**: The instance does not exist yet — the guard runs before creation. Use the `next(vm => ...)` callback.
...
- **Setup stores instead of option stores**: This project uses option stores (`state` / `getters` / `actions`) to match Options API components. Setup stores also lose `$reset` for free.
...
- **`mapState` used for a value the component assigns to**: `mapState` is read-only — writes fail silently. Use `mapWritableState`, or an action.
- **Store accessed via `useStore()` inside `methods` on every call**: Map it once with `mapState`/`mapActions`, or cache the store on the instance — repeated lookups obscure the dependency.
...
Nuxt's data layer (`useAsyncData`, `useFetch`, `useRuntimeConfig`) is composable-only with no imperative equivalent. These checks apply to Nuxt projects, where a `setup()` option is the accepted bridge — they do not license Composition API elsewhere.

- **Browser-only API used without `import.meta.client` guard or `mounted()`**: `window`, `document`, `localStorage` crash the server build. In Options API, `mounted` never runs on the server, so it is the natural guard.
...
- **Expensive work in a `methods` function called from the template**: Methods re-run on every render with no caching. Anything derived from state belongs in `computed`.
...
- **Missing `markRaw()` for large immutable structures in `data()`**: `data()` deep-proxies everything. Wrap giant read-only datasets and third-party instances in `markRaw()`.
...
- **Hand-rolled debounce with a bare `setTimeout` in a watcher**: The timer handle must live on the instance and be cleared in `beforeUnmount`, or the callback fires after teardown. Prefer a debounce helper created once in `created` (`this.search = debounce(this.doSearch, 300)`) and cancelled in `beforeUnmount`.

### MEDIUM — Component API Style

- **Composition API in new code**: This project standardizes on the Options API. New components use `<script>` with `export default defineComponent({ ... })` — not `<script setup>`, `setup()`, `ref`, `reactive`, `computed()`, or `defineProps`/`defineEmits` macros.
- **A `setup()` option used as an escape hatch without justification**: Acceptable only for a library that exposes no imperative alternative (see the Options API bridge notes in `rules/vue/patterns.md`). Keep it to one thin component and comment why.
- **Missing `defineComponent()` wrapper in a TypeScript SFC**: Without it, `this` inside `computed`, `watch`, `methods`, and lifecycle hooks is untyped.
- **`expose` option omitted on a component whose parent uses a template ref**: Without it the parent reaches every internal method and data property. Declare the intended public API.
- **Component over 300 lines (template + script)**: Extract subcomponents or shared modules.
- **Deep mixin chains**: Mixins including mixins makes the merged option set unreadable. Keep the chain flat — one level.
...
npx eslint . --rule 'vue/order-in-components: warn'    # enforce Options API option order
npx eslint . --rule 'vue/require-name-property: warn'
npx prettier --check .
npm audit

# Options API compliance — any hit is a finding
rg -l 'script setup' -g '*.vue'
rg -n 'defineProps|defineEmits|defineModel|\bsetup\s*\(' -g '*.vue'
```
