...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        Schema::table('rstl_monthly_breakdowns', function (Blueprint $table) {
            ...
            $table->unsignedInteger('genderqueer_count')->default(0)->after('non_binary_count');
            $table->unsignedInteger('non_binary_led_count')->default(0)->after('genderqueer_count');
            $table->unsignedInteger('genderqueer_led_count')->default(0)->after('non_binary_led_count');
        });
        ...
        Schema::table('program_funding_summaries', function (Blueprint $table) {
            ...
            $table->decimal('non_binary_amount', 20, 6)->default(0)->after('non_binary_projects');
            $table->unsignedInteger('genderqueer_projects')->default(0)->after('non_binary_amount');
            $table->decimal('genderqueer_amount', 20, 6)->default(0)->after('genderqueer_projects');
        });
    }
    ...
    {
        Schema::table('program_funding_summaries', function (Blueprint $table) {
            $table->dropColumn(['non_binary_projects', 'non_binary_amount', 'genderqueer_projects', 'genderqueer_amount']);
        });

        Schema::table('rstl_monthly_breakdowns', function (Blueprint $table) {
            $table->dropColumn(['non_binary_count', 'genderqueer_count', 'non_binary_led_count', 'genderqueer_led_count']);
        });
