...
class ReportYearDemoSeeder extends Seeder
{
    /**
     * Reporting year this demo data describes.
     *
     * The year is the only thing that moves between runs, so it lives here
     * rather than being repeated across the title, the dates and the school
     * year. Re-running for a different year adds that year; it leaves earlier
     * demo years in place.
     */
    ...
    public function run(): void
    {
        ...
        DB::transaction(function (): void {
            $reportYear = ReportYear::query()->updateOrCreate(
                ['year' => self::YEAR],
                [
                    'title' => self::YEAR.' Sex Disaggregated Data Report',
                    'description' => 'Data report for '.self::YEAR.' covering DOST IX employees, RSTL services, and categorized SETUP, CEST, and GIA programs.',
            ...
            /*
             * GFPS members per status, a subset of the employees above: the two
             * are entered independently, so these are smaller but need not
             * reconcile with anything.
             */
            foreach ([
                'plantilla' => ['female_count' => 11, 'male_count' => 4],
                'cos' => ['female_count' => 9, 'male_count' => 2],
                'jo' => ['female_count' => 2, 'male_count' => 0],
            ] as $slug => $values) {
                GfpsMemberStatusBreakdown::query()->updateOrCreate(
                    [
                        'report_year_id' => $reportYear->id,
                        'employment_status_id' => $employmentStatuses[$slug],
                    ],
            ...
            $schoolYear = SchoolYear::query()->where('name', self::YEAR.'-'.(self::YEAR + 1))->first();

            ScholarshipSummary::query()->updateOrCreate(
                ['report_year_id' => $reportYear->id],
                [
                    'school_year_id' => $schoolYear->id ?? null,
                    'as_of_date' => fake()->dateTimeBetween(self::YEAR.'-01-01', self::latestPlausibleDate())->format('Y-m-d'),
    ...
    /**
     * The last date the demo data may claim: the end of the reporting year, or
     * today when that year has not finished yet. Without the clamp, seeding the
     * current year stamps records in the future.
     */
    private static function latestPlausibleDate(): string
    {
        return min(self::YEAR.'-12-31', now()->toDateString());
    }

    /**
     * Annual reports are published late in their own year, so that is the window
     * used — pulled back to today when the year is still running, which would
     * otherwise leave the start of the window after its end.
     */
    private static function publishedAt(): DateTime
    {
        $end = self::latestPlausibleDate();

        return fake()->dateTimeBetween(min(self::YEAR.'-11-01', $end), $end);
    }
}
