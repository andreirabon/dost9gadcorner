...
const selectedCategory = computed<FundingCategorySummaryData | null>(() => props.categories.find((row) => row.slug === selectedSlug.value) ?? null);
...
const buttonClass = (isActive: boolean): string[] => [
    'inline-flex items-center rounded-lg border px-3.5 py-2 text-base font-semibold tracking-wide transition-[transform,background-color,border-color,color] duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-400 active:scale-[0.97]',
    isActive
        ? 'border-brand-500 bg-brand-600 text-white shadow-sm'
        : 'border-brand-800 bg-brand-900 text-brand-100 hover:border-brand-700 hover:bg-brand-800 report-light:border-slate-300 report-light:bg-slate-50 report-light:text-slate-700 report-light:hover:border-slate-400 report-light:hover:bg-slate-100',
];
</script>
